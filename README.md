...
Runs 4-stage adversarial evaluation: (1) compares linguistic certainty vs. KLE uncertainty, (2) penalizes unaddressed contradicting FHIR facts, (3) flags omitted differentials from literature, (4) retrieves similar past diagnostic errors from DuckDB/Supabase via **HNSW vector search** then **neural re-ranking** — final score = `0.5×clinical_relevance + 0.3×recency_decay + 0.2×feedback` (clinical_relevance considers severity, KLE similarity, diagnostic keyword overlap, CheXbert label match; recency uses 180-day exponential decay). Gated by `ENABLE_PAST_MISTAKES_RERANKING`. Doctor-rejected diagnoses (where agents might have hallucinated) are auto-inserted into this database for future retrieval so that the system learns from its mistakes. Optional 5th stage uses MedGemma for deeper semantic analysis.
